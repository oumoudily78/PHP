<?php
require_once 'partie_php/config.php';

$recherche_produits = [];
$search = isset($_GET['search']) ? $_GET['search'] : '';

if (!empty($search)) {
    try {
        // On cherche dans tous les produits, peu importe la catégorie
        $sql = "SELECT p.*, c.label as cat_nom 
                FROM produits p 
                JOIN categories c ON p.categorie_id = c.id 
                WHERE p.nom LIKE :search OR p.description LIKE :search";
        
        $stmt = $pdo->prepare($sql);
        $stmt->execute(['search' => "%$search%"]);
        $recherche_produits = $stmt->fetchAll();
    } catch (Exception $e) {
        $recherche_produits = [];
    }
}
?>
<!DOCTYPE html>
<html lang="fr">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Dashboard Boutique</title>
    <link rel="stylesheet" href="style.css">
</head>
<body>
    <header class="head">
        <h2>🛍 Omnistock Vesta</h2>
    </header>

    <div class="dashboard">

        <aside class="sidebar">
                <h2>🛍 Omnistock Vesta</h2>
                <ul>
                    <li class="menu-btn active">🏠 Accueil</li>
                    <li><a href="administration.php"><button class="menu-btn active">⚙️ Administration</button></a></li>
                </ul>
                <h3>Bienvenue 🖐</h3>
        </aside>
        <main class="main-content">
            <h1>Gestion des Produits</h1>
            <div class="zone_recherche">
                <input type="text" placeholder="Rechercher une catégorie ou un produit...">
                <button class="bouton_ok"><a href="supp-details/rechercher.html">OK</a></button>
            </div>
            <h3>Categorie</h3>

            <div class="category-tabs">
                <button class="card"><a href="pc.html">PC</a></button>
                <button class="card"><a href="voile.html">Voile</a></button>
            </div>
                <div class="produit_zone">
                    <div class="carte">
                        <img src="Image/PC/DELL.jpeg" alt="">
                        <h3>Dell xps</h3>
                        <span class="price">850000 FCFA</span>
                        <button>Voir plus</button>
                    </div>
                    <div class="carte">
                        <img src="Image/PC/loq.jpeg" alt="">
                        <h3>Lenovo loq</h3>
                        <span class="price">750000 FCFA</span>
                        <button>Voir plus</button>
                    </div>
                    <div class="carte">
                        <img src="Image/PC/lenovo legion.jpeg" alt="">
                        <h3>lenovo legion</h3>
                        <span class="price">850000 FCFA</span>
                        <button>Voir plus</button>
                    </div>
                    <div class="carte">
                        <img src="Image/voile/jersey noir.jpeg" alt="">
                        <h3>Jersey Noir</h3>
                        <span class="price">6500 FCFA</span>
                        <button>Voir plus</button>
                    </div>
                    <div class="carte">
                        <img src="Image/voile/jersey saumon.jpeg" alt="">
                        <h3>Jersey Saumon</h3>
                        <span class="price">6500 FCFA</span>
                        <button>Voir plus</button>
                    </div>
                    <div class="carte">
                        <img src="Image/voile/jersey beige.jpeg" alt="">
                        <h3>Jersey Beige</h3>
                        <span class="price">6500 FCFA</span>
                        <button>Voir plus</button>
                    </div>
                </div>
        </main>
    </div>
    <footer class="footer">
        @$ 2026 🛍 Omnistock Vesta - tous droits réservés
        Gestion de produit<br>
        email: groupe8@gmail.com<br>
        numero: 76 87 97 74<br>
        yuta@okkotsu<br>
    </footer>
</body>
</html>